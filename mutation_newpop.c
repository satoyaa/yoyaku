#include <stdio.h>
/*
 突然変異: 新規個体生成 (new population)
 - 集団の後半（通常は下位個体群）をランダムに初期化して新しい個体を生成する。
 - 観光地列をランダムに選んで配置し，分岐(`dest`)や出発時刻(`time`)をランダムに設定する。
 - 予約情報も初期化またはランダムに設定し，多様性を導入して局所最適からの脱出を狙う。
 - 条件により突然変異をスキップする場合がある。
*/
#include <stdlib.h>
#include <string.h>
#include "extern.h"
