#include <stdio.h>
/*
 交叉: Two-point crossover
 - 個体をペアにして，それぞれの遺伝子列の2点区間を交換する二点交叉を実施する。
 - 交叉確率に基づいてペア毎に交叉を行うか判定する。
 - 交換後に分岐インデックス(`dest`)のオフセット調整や範囲外チェックを行い，遺伝子の整合性を保つ。
 - 予約情報の交叉は後回しにしている（必要なら実装を拡張）。
*/
#include <stdlib.h>
#include <math.h>
#include <time.h>
